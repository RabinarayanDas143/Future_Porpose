...
public class OperationUsingModelClass14 {
	public static void main(String[] args) {
		...
		Employee map5 = empList.stream().sorted((a, b) -> a.getYearOfJoining() - b.getYearOfJoining()).findFirst()
				.get();
		// System.out.println(map15);

		// Print Average age of Male and Female Employees
		Map<String, Double> map6 = empList.stream().collect(
				Collectors.groupingBy(e -> e.getGender().toString(), Collectors.averagingLong(e -> e.getAge())));
		...
		Employee map8 = empList.stream().sorted((a, b) -> b.getAge() - a.getAge()).findFirst().get();
		...
		Employee map10 = empList.stream().filter(e -> e.getDeptName().equalsIgnoreCase("hr")).findFirst().get();
		...
		long map16_totalsalary = empList.stream().collect(Collectors.summingLong(e -> e.getSalary()));
		double map17_averageSalary = empList.stream().collect(Collectors.averagingDouble(e -> e.getSalary()));
//		System.out.println("map16_totalsalary :" + map16_totalsalary);
//		System.out.println("map17_averageSalary :" + map17_averageSalary);

		// Print Average salary of each department
		Map<String, Double> map18 = empList.stream()
		...
		Map<String, Object> map19 = empList.stream()
		...
		List<Long> map20 = empList.stream().map(e -> e.getSalary()).sorted().collect(Collectors.toList());
		// System.out.println(map20);

		// Print list of employee's second highest salary based on department
		Map<String, Long> map21 = empList.stream()
