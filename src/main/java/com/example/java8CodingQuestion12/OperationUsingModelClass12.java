...
public class OperationUsingModelClass12 {
	public static void main(String[] args) {
		...
		ModelClass map9 = empList.stream().sorted((a, b) -> b.getAge() - a.getAge()).findFirst().get();
		...
		List<ModelClass> map11 = empList.stream().filter(e -> e.getAge() > 30).collect(Collectors.toList());

		// Find the department name which has the highest number of employees
		Entry<String, Long> map12 = empList.stream()
		...
		ModelClass map13 = empList.stream().filter(e -> e.getDeptName().equalsIgnoreCase("HR")).findAny()
				.orElseThrow(() -> new RuntimeException("Requested resource is Not found.."));
		// System.out.println(map13);

		// Find the department names where the number of employees in the department is
		// over 3
		List<Entry<String, Long>> map14 = empList.stream()
				.collect(Collectors.groupingBy(e -> e.getDeptName(), Collectors.counting())).entrySet().stream()
		...
		List<String> map15 = empList.stream().map(e -> e.getDeptName()).distinct().collect(Collectors.toList());
		...
		List<ModelClass> map16 = empList.stream().filter(r -> r.getCity().equalsIgnoreCase("Blore"))
		...
		ModelClass map20 = empList.stream().sorted((a, b) -> b.getYearOfJoining() - a.getYearOfJoining()).findFirst()
				.get();
		// System.out.println(map20);

		// Print average and total salary of the organization.
		double map21_totalSalary = empList.stream().collect(Collectors.summingDouble(e -> e.getSalary()));
		double map22_averageSalary = empList.stream().collect(Collectors.averagingDouble(e -> e.getSalary()));
		// System.out.println(map21_totalSalary);
		// System.out.println(map22_averageSalary);

		// Print Average salary of each department
		Map<Object, Double> map23 = empList.stream()
		...
		ModelClass map24 = empList.stream().sorted((a, b) -> ((Long) b.getSalary()).compareTo(a.getSalary()))
				.findFirst().get();
		// System.out.println(map24);

		// Find Second Highest salary in the organisation
		ModelClass map25 = empList.stream().sorted((a, b) -> ((Long) b.getSalary()).compareTo(a.getSalary())).skip(1)
				.findFirst().get();
		...
		Map<String, ModelClass> map27 = empList.stream()
		...
		List<Long> map28 = empList.stream().map(e -> e.getSalary()).sorted((a, b) -> ((Long) a).compareTo(b))
				.collect(Collectors.toList());
		// System.out.println(map28);

		// Print list of employee's second highest salary based on department
		Map<String, ModelClass> map29 = empList.stream()
				.collect(Collectors.groupingBy(e -> e.getDeptName(),
						Collectors.collectingAndThen(Collectors.toList(), e -> e.stream()
								.sorted((a, b) -> ((Long) b.getSalary()).compareTo(a.getSalary())).skip(1).findFirst().get())));
